...
def Data_Preprocessing(DF1, DF2): 
    ...
    #    df_N_1 = sum(df['County_State'].dropna().apply(lambda x: len(x.split(', ', 1))) != 2)/len(df) # detect any value cannot split into county and state
        ...
        df_N_1 = sum(df['County_State'].dropna().apply(lambda x: x.lower().find(' parish') >= 0))/len(df) + df_N_1 # detect inconsistent county name
        df_N_1 = sum(df['County_State'].dropna().apply(lambda x: x.lower().find(' county') >= 0))/len(df) + df_N_1 # detect inconsistent county name
        df_N_1 = sum(df['County_State'].dropna().apply(lambda x: x.lower().find(' census area') >= 0))/len(df) + df_N_1 # detect inconsistent county name
        df_N_1 = sum(df['County_State'].dropna().apply(lambda x: x.lower().find('st.') >= 0))/len(df) + df_N_1 # detect inconsistent county name
        ...
        df_N_5 = No_Mid_Check(df.NUninsured_CI_LowerBound, df.Number_Uninsured, df.NUninsured_CI_UpperBound)/len(df)+df_N_5 # detect any number out of its range
    ...
    def Cleanliness1(df1):
        ...
        df1_N_3 = sum(df1['State'].dropna().apply(lambda x: (not x.isalpha()) or len(x) != 2))/len(df1) # detect any value without a 2-alpha format
        df1_N_3 = sum(df1['State'].dropna().apply(lambda x: x.upper() not in State_List))/len(df1) + df1_N_3 #detect any wrong state value
        ...
        df1_N_5 = No_Mid_Check(df1.Lower_Payment_Est, df1.Ave_Payment, df1.Higher_Payment_Est)/len(df1)+df1_N_5 # detect any number out of its range
...
#            'NUninsured_CI_UpperBound']].boxplot()  # The top value belongs to Los Angeles County in 2013 and 2015 year, it is not a wrong value
        # Check the number of insured is within the 90% confidence interval, drop rows with wrong values
        df1 = Mid_Check(df1, df1.NInsured_CI_LowerBound, df1.Number_Insured, df1.NInsured_CI_UpperBound)    
        # Check the number of uninsured is within the 90% confidence interval, drop rows with wrong values
    ...
    def Cleaning_2(DF2):
        ...
        # Remove wrong state
#        DF2[DF2['State'].apply(lambda x: x in State_List)]
        df2 = df2[df2['State'].apply(lambda x: x.upper() in State_List)]
        ...
        # Check the average payment of a certain care is with the maximum-minimum payment range, drop rows with wrong values
...
def Histogram(df_all):
    ...
    for i in range(len(all_names)):    
        tracei = go.Histogram(x=df_all[all_names[i]],name = all_names[i])
...
def Corr_Cluster(df_all):
    ...
    def encoding(df_all_wyf):
        df_all_wyf['Regions']=df_all_wyf['Regions'].replace(['southeast'], '0')
        df_all_wyf['Regions']=df_all_wyf['Regions'].replace(['midwest'], '1')
        df_all_wyf['Regions']=df_all_wyf['Regions'].replace(['southwest'], '2')
        df_all_wyf['Regions']=df_all_wyf['Regions'].replace(['northeast'], '3')
        df_all_wyf['Regions']=df_all_wyf['Regions'].replace(['west'], '4')
        df_all_wyf['Regions']=df_all_wyf['Regions'].astype('int')
        return df_all_wyf
        
    #Encode the columns 'Regions' in df_all_wyf. 
    df_all_wyf=df_all.copy()
    df_all_wyf=encoding(df_all_wyf)
    ...
    def plotlyCorr( corr_val):
        ...
        a = ['lightskyblue','lightskyblue','lightskyblue','white','white','white','white','white','white','white','white','white']
        b = ['white','white','white','lightskyblue','lightskyblue','lightskyblue','white','white','white','white','white','white']
        c = ['white','white','white','white','white','white','white','white','lightskyblue','lightskyblue','lightskyblue','white']
        ...
        trace = go.Table(
            ...
            cells=dict(values=[corr_val.names, corr_val.Number_Insured, corr_val.NInsured_CI_LowerBound, corr_val.NInsured_CI_UpperBound, 
                       ...
                       align = 'center',
                       fill = dict(color=['white',a,a,a,b,b,b,'white','white',c,c,c,'white'])))
    ...
    def plotlyCorr_sub( corr_val ):
        #Plotly correlation table:
        names1=['names','Insured_rate','Ave_Payment','Population', 'Regions']
        trace1 = go.Table(
            type = 'table',
            header=dict(values=names1,
                        align = 'center'),
            cells=dict(values=[corr_val.names,corr_val.Insured_rate,corr_val.Ave_Payment,corr_val.Population,corr_val.Regions],
                       align = 'center',
            fill = dict(color=['white','lightskyblue','white','white','white'])))
    ...
    corr_full=getCorr(df_all_wyf)
    plotlyCorr(corr_full)
    corr_sub=getCorr_sub(df_all_wyf)
    plotlyCorr_sub(corr_sub)
...
#%% Network Analysis
df_all1 = df_all.sort_values(by = ["Regions","State"])
...
def Net_work(df):
    ...
    def add_nodes(dataframe):
        ...
        for i in range(len(dataframe)):
            G.add_node(dataframe["Population"][i], pos = (30,(dataframe["Population"][i]- min2)*1800/(max2 - min2)))
    ...
    def add_edges(dataframe):
        ...
        for i in range(len(dataframe)):
            G.add_edge(dataframe["County_State"][i], dataframe["Insured_rate"][i],pos = i)
        # Add edges between insured rate and medical payment
        for i in range(len(dataframe)):
            G.add_edge(dataframe["Insured_rate"][i], dataframe["Ave_Payment"][i],pos = i+len(dataframe))
        # Add edges between medical payment and population
        for i in range(len(dataframe)):
            G.add_edge(dataframe["Ave_Payment"][i], dataframe["Population"][i],pos = i+2*len(dataframe))
        # Add edges between county and regions
        for i in range(len(dataframe)):
            G.add_edge(dataframe["County_State"][i], dataframe["Regions"][i],pos = i+3*len(dataframe))
        # G.edges()
    ...
    def global_metrics(df):
        ...
        ave_centrality = np.mean(list(centrality.values()))
        print();
        print("Average closeness centrality of this network")
